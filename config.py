...
# Снимки отчёта лежат файлами на сервере: мастер прислал — бот сразу скачал.
# Путь относительный отсчитывается от рабочего каталога службы.
PHOTO_DIR = _get("PHOTO_DIR", "photos")
# Сколько дней держать снимки. К этому сроку отчёт давно в CRM.
PHOTO_KEEP_DAYS = _get_int("PHOTO_KEEP_DAYS", 14)

...
| Коммит | `2f58c39` |
| Всего коммитов в репозитории | 12 |
...
1. docs_photo   «Загрузите фото БСО и чека СМЗ»          [Готово]
                несколько снимков; каждый скачивается и сразу
                удаляется из чата, счётчик дописывается в вопрос
...
- **Telegram.** Ни одного теста против настоящего API: всё на моках. Ограничения
  частоты, удаление старых сообщений, права в чатах не проверяются.
...
2. **Два брошенных отчёта в бою.** У одного мастера висят незавершённые отчёты по
   двум заявкам с 18 и 21 сентября. Пока висят, новый отчёт по этим заявкам не
   заводится — бот отвечает «Сначала завершите предыдущий отчёт».
...
|---|---|---|
| 24.09.2026 | `2f58c39` | Первая сборка документа. Зафиксировано состояние после выкладки 23.09: разделение ролей, вечерняя сводка сбоев, переписанный отчёт мастера с проведением заявки в CRM, снимки файлами на диске. |

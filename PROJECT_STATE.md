...
2. **Два брошенных отчёта в бою.** У одного мастера висят незавершённые отчёты
   по двум заявкам с 18 и 21 сентября: он нажал кнопку и не ответил ни на один
   вопрос. Обе заявки потом провёл администратор вручную, в CRM они закрыты,
   так что новый отчёт по ним никому не понадобится.

   До 24.09 эти строки были ловушкой: фото, присланное мастером в чат **не**
   реплаем, уходило в последний незавершённый отчёт. В чате оно исчезало, в CRM
   не попадало никогда, мастер об этом не узнавал. Причина устранена — фото
   теперь требует реплая так же, как текст, — но сами строки остались и ждут
   решения заказчика, чистить их или нет.
...
| 24.09.2026 | `<этот коммит>` | Фото в отчёте засчитывается только реплаем на вопрос бота — как текст. До этого снимок, присланный в чат просто так, уходил в последний незавершённый отчёт мастера: в чате исчезал, в CRM не попадал. Уточнён раздел 11 про два брошенных отчёта — обе их заявки давно проведены администратором вручную. |

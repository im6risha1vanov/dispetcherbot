...
| Коммит | `<см. журнал>` |
| Всего коммитов в репозитории | 17 |
...
├── access.py               ~200 Кто что вправе: один список на справку и на права
├── migrate.py              ~160 Применение миграций с отметкой в schema_migrations
...
CREATE TABLE public.shifts (
    ...
    paused_at timestamp with time zone,   -- «Занят» до конца дня
...
| 024 | `shifts.paused_at` — дневная отметка «Занят» |
...
Миграции применяет `migrate.py`: неприменённые по возрастанию номера, каждую в
своей транзакции вместе с отметкой в `schema_migrations`. `--status` показывает
состояние, `--baseline NNN` отмечает уже применённое, не выполняя заново.
Обе службы при старте отказываются работать на базе с неприменёнными миграциями
и пишут владельцу, что применить.
...
Разбор ячеек по индексам: 0 — ID, 4 — время заявки, 5 — тип, 6 — статус,
7 — имя клиента, 8 — адрес, 9 — мастер, 10 — создано (лок), 11 — закрыто (лок),
12 — сумма. Колонки 1–3 (пустая, «П», «Н») не читаются.
Суффикс `(Отз)` в статусе означает отзывную заявку и срезается регуляркой.

**Заголовки сверяются перед разбором.** Ожидаемые подписи колонок 0, 4–12 сняты
с живой CRM 24.09.2026 и лежат в `GRID_COLUMNS`. Расхождение по подписи или по
числу колонок, как и «строки есть, а разобрано ноль», поднимает `CrmLayoutError`
и **прерывает разбор целиком** — ни одной строки не возвращаем. Пропускать
нельзя: перестановка колонок при том же их числе уронит адрес в поле мастера,
и ничего не упадёт. Тревога уходит сразу, минуя порог «5 неудач подряд»,
и повторяется не чаще раза в сутки.
...
**Гарантия идёт мимо очереди.** Она уходит тому, кто делал работу, даже если он
утром не отмечался или помечен «Занят»: и то и другое — правила очереди, а
гарантия к распределению отношения не имеет. Занят живой заявкой — ждём его.
Если он выключен, его нет в справочнике или ему некуда доставить — заявка
уходит в общую очередь с пометкой «первым был». До 24.09 в этих случаях она
ждала вечно и не доставалась никому.

Повтор остаётся при прежнем правиле: к тому же мастеру, но только если он в
сегодняшней очереди и свободен, иначе по кругу.
...
| `DIGEST_TIME` 22:00 | Итоги дня владельцу, директору и в общий чат мастеров | Отправится при первом опросе после запуска |
...
1. **Четыре отчёта застряли в состоянии «подтверждён».** Администратор их
...
2. **Снимки, съеденные брошенным отчётом.** До 24.09 фото, присланное мастером
...
3. **`.env.example` отстал от `config.py`.** Не описаны `CRM_ALLOW_CLOSING`,
...
4. **`git` на сервере тянет только `main`.** Refspec узкий, поэтому
...
5. **Медленный SSH-баннер на сервере.** Ответ приходит дольше 40 секунд, похоже на
   обратный DNS-запрос в настройках sshd. Ни на что не влияет, кроме удобства, но
   ломает инструменты с коротким таймаутом.

6. **Бот и поллер держат разные HTTP-сессии к CRM.** Два процесса — два входа.
...
- **`GRID_COLUMNS`.** Подписи колонок зашиты в код и сверяются при каждом опросе.
  Это защита, но и хрупкость: переименование колонки в CRM останавливает разбор
  целиком, пока список не поправят.
...
| 24.09.2026 | `2e0d49d` | Убраны два брошенных отчёта (18.09 и 21.09) — переведены в `discarded`, незавершённых отчётов в базе не осталось. При проверке найдены четыре отчёта, застрявших в состоянии «подтверждён»: все четыре заявки — отказы, расхождений по деньгам нет, строки оставлены как достоверная запись. Подтверждён фактом вред от приёма фото без реплая: пять снимков сохранной расписки по заявке от 18.09 в CRM не попали. |
| 24.09.2026 | `<этот коммит>` | Семь доработок. Учёт миграций: `schema_migrations`, `migrate.py`, отказ служб стартовать на устаревшей базе. Сверка заголовков грида CRM с остановкой разбора и тревогой без порога. `/help` по ролям из единого списка прав в `access.py`, роль определяется по человеку, а не по чату; заодно закрыта дыра в `/master_chat`, который не проверялся вовсе. Итоги дня дополнительно в общий чат мастеров. Понятный ответ выключенному мастеру на «+». Панель дня `/today` с «Занят»/«В работе» и включением выключенных директором. Гарантия идёт мимо очереди: активному мастеру независимо от отметки и «Занят», выключенному — по кругу вместо вечного ожидания. |
